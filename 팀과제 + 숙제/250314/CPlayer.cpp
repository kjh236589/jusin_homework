...
CPlayer::CPlayer() : m_pBullet(nullptr), left(false), b_jump(false), coolTime(GetTickCount64()), boomCoolTime(GetTickCount64()), item(BASIC), angle(0), boom(10), bullets(0), theta(0.f), gunX(0.f), gunY(0.f), angle2(0)
{
}
	...
	if (GetAsyncKeyState('Z'))
	{
		...
		case BASIC:
			if (coolTime + 200 <= GetTickCount64()) {
				CObj* pBullet = CAbstractFactory<CBullet>::Create_Obj(gunX, gunY);
				pBullet->Set_Size(5.f, 5.f);
				dynamic_cast<CBullet*>(pBullet)->SetSpeed((gunX - Get_Info()->fX) / 50.f, (gunY - Get_Info()->fY) / 50.f);
